...
class ZernikeNaive(object):
    ...
    def R_nm(self, n, m, rho):
        ...
        if (n - m) % 2 != 0:
            ...
            for j in range(int((n - m) / 2) + 1):
                coef = ((-1) ** j * fact(n - j)) / (fact(j) * fact(((n + m) / 2 - j).astype(np.int32)) * fact(
                    ((n - m) / 2 - j).astype(np.int32)))
                r += coef * rho ** (n - 2 * j)
            return r
    ...
    def Z_nm(self, j, rho, theta, normalize_noll, mode):
        """
        ...
        is_even = (j % 2 == 0)  # True if even, False otherwise
    ...
    def evaluate_series(self, rho, theta, normalize_noll, mode, print_option='Result'):
        ...
        for n in range(n_max + 1):  # Loop over the Zernike index
            for m in np.arange(-n, n + 1, 2):
                start = tm()
                Z, _ = self.Z_nm(n, m, rho, theta, normalize_noll, mode)
